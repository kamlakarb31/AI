...
class DataSet:
    ...
    def getPluralityValue(self): # returns a plurality value with maximum occurances of answer        
        ...
        print("Plularity set :" , d)
        keys = list(d.keys())
        keys.sort(reverse=True)
        print("After numeric sort, keys are : ",keys)
        if len(keys) >1 and keys[0] != keys[1]:
            print(" Reurning ", d[keys[0]])
            return d[keys[0]]
        else:
    ...
    def infoGain(self, feature) : # finds info gain of a particular feature
        if feature in self.dataset.keys() :
            featurevalues = self.dataset[feature]
            total = len(featurevalues)
            featurevalueset = set(featurevalues)
            d = dict.fromkeys(featurevalueset, 0)
            for item in featurevalues :
                d[item] = d[item]+1
            branches = self.splitOnFeature(feature)
            gain = 0
            for key in branches :               
                dsobj = branches[key]
...
def calculateAns(dsobj, feature, pluralValue, descr ):   
    ...
    for key in list(branches.keys()):
        ...
        if (newdsobj.uniqueAns() == 1):
            print(descr+"---" +feature , " with value =", key , " --- ", newdsobj.dataset['Ans'][0])
        elif(newdsobj.uniqueAns() == 0):
            print(descr+"---" +feature , " with value =", key , " --- ", pluralValue)            
